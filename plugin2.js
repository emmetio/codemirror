 ...
 */
export function globalSetup(CodeMirror) {
    /**
     * Runs Emmet action
		...
		if (!CodeMirror.commands[command]) {
            CodeMirror.commands[command] = (cm) => {
                let result = singleSelectionActions.indexOf(action) === -1
                    ? runForSingleCursor(action, cm, runAction)
                    : runForMultipleCursors(action, cm, runAction);
 ...
 * @return {Function}
 */
function runForSingleCursor(name, cm, run) {
    var result;
    cm.operation(() => result = run(name, new EmmetEditor(cm)));
    return result;
};
 ...
 * @return {Function}
 */
function runForMultipleCursors(name, cm, run) {
